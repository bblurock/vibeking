...
class FluidAudioBridgeInternal {
    ...
    // Streaming ASR running transcript, maintained by a background consumer of
    // `SlidingWindowAsrManager.transcriptionUpdates`. The recognizer decodes on
    // its OWN task, so reading confirmed/volatile inline right after queuing a
    // buffer always races ahead of processing (it read empty). Instead we drain
    // the update stream — which fires exactly when a window finishes decoding —
    // and store the latest running transcript here for `feed`/`finish` to return.
    private var streamingConsumerTask: Task<Void, Never>?
    private var streamingLatestText: String = ""
    private let streamingTextLock = NSLock()

    private func setLatestStreamingText(_ t: String) {
        streamingTextLock.lock()
        streamingLatestText = t
        streamingTextLock.unlock()
    }
    private func latestStreamingText() -> String {
        streamingTextLock.lock()
        defer { streamingTextLock.unlock() }
        return streamingLatestText
    }
    ...
    /// Batch transcribe that also returns per-token timings as a JSON array
    /// (`[{"t":token,"s":startSec,"e":endSec}, …]`). Used by the live-preview
    /// frozen-head/live-tail stitcher to map text back to audio positions. The
    /// underlying decoder already computes these timings; the standard
    /// `transcribeSamples` just drops them.
    func transcribeSamplesTimed(_ samples: [Float]) throws -> (String, String) {
        ...
        let timed = (r.tokenTimings ?? []).map {
            TimedToken(t: $0.token, s: $0.startTime, e: $0.endTime)
        }
        let json = (try? JSONEncoder().encode(timed)).flatMap { String(data: $0, encoding: .utf8) } ?? "[]"
        return (r.text, json)
    }
    ...
    /// Low-latency LIVE-PREVIEW config. The `.default` preset only processes its
    /// first window after chunk+right = 15+2 = 17 s of audio accumulate, so any
    /// dictation shorter than that shows NO preview at all. vibeking derives its
    /// authoritative final from a separate batch decode (with hotword/context
    /// bias), so this preview trades per-window accuracy for latency: a small
    /// chunk + tiny right-context surfaces words within ~1.3 s. `volatileTranscript`
    /// updates on every processed window regardless of confidence, so the live
    /// tail flows immediately.
    private var streamingLiveConfig: SlidingWindowAsrConfig {
        SlidingWindowAsrConfig(
            // 1.0 s windows decode whole words cleanly and cross few boundaries,
            // so the running transcript doesn't fragment ("truncated" → "d. ed.")
            // or drop phrases the way sub-second windows did. The cost is cadence:
            // text grows in ~1 s steps. Each window's text is promoted into the
            // append-only `confirmedTranscript` prefix (eager confirmation) so the
            // transcript actually accumulates instead of showing only the latest
            // ~1 word. Preview-only — the authoritative final is a separate batch
            // decode with hotword/context bias.
            chunkSeconds: 1.0,
    ...
    func initializeStreamingAsr() throws {
        ...
        Task {
            do {
                // Only download/compile + cache the models here. The streaming
                // MANAGER is created fresh per session in `streamingAsrStart`,
                // because SlidingWindowAsrManager's input AsyncStream is
                // single-use — `finish()` terminates it permanently, so a reused
                // manager's recognizer would listen to a dead stream and receive
                // no audio (the bug this replaces).
                let models = try await AsrModels.downloadAndLoad()
                self.asrModels = models
            } catch {
                initError = error
    ...
    func streamingAsrStart() throws {
        ...
        // Tear down any consumer from a previous session.
        streamingConsumerTask?.cancel()
        streamingConsumerTask = nil
        setLatestStreamingText("")

        let semaphore = DispatchSemaphore(value: 0)
        var startError: Error?
        let config = streamingLiveConfig

        Task {
            do {
                // Fresh manager + fresh input AsyncStream for this session.
                let manager = SlidingWindowAsrManager(config: config)
                try await manager.loadModels(models)
                try await manager.startStreaming(source: .microphone)
                self.streamingAsrManager = manager

                // Drain transcription updates: each fires when a window finishes
                // decoding; we then read the manager's cumulative confirmed +
                // volatile as the running transcript, so `feed` never races ahead
                // of processing.
                self.streamingConsumerTask = Task { [weak self] in
                    let updates = await manager.transcriptionUpdates
                    for await _ in updates {
                        let confirmed = await manager.confirmedTranscript
                        let volatile = await manager.volatileTranscript
                        let running = [confirmed, volatile].filter { !$0.isEmpty }.joined(separator: " ")
                        self?.setLatestStreamingText(running)
                    }
                }
            } catch {
                startError = error
        ...
        if let error = startError {
            FileHandle.standardError.write(
                "[bridge][DIAG] streamingAsrStart ERROR \(error)\n".data(using: .utf8)!)
            throw error
        }
    }

    // Returns the running hypothesis after feeding this chunk: the locked
    // `confirmedTranscript` prefix + the live `volatileTranscript` tail. The
    // confirmed prefix only grows, so a frontend prefix-diff keeps it solid and
    // only the volatile tail moves — flicker-free streaming, vs the old void
    // return that forced callers to re-decode the whole buffer for a preview.
    func streamingAsrFeed(_ samples: [Float]) throws -> String {
        ...
        Task {
            ...
            // Fire-and-forget: just queue the audio. The background consumer
            // updates the running transcript when this (or an earlier) window
            // finishes decoding.
    ...
    func streamingAsrFinish() throws -> String {
        ...
        // Stop draining updates; the session is closing.
        streamingConsumerTask?.cancel()
        streamingConsumerTask = nil
    ...
    func isStreamingAsrAvailable() -> Bool {
        // Models cached = ready to spin up a fresh per-session manager. (The
        // manager itself is created in `streamingAsrStart`, not at init.)
...
@_cdecl("fluidaudio_transcribe_samples_timed")
public func fluidaudio_transcribe_samples_timed(
    _ ptr: UnsafeMutableRawPointer?,
    _ samples: UnsafePointer<Float>?,
    _ sampleCount: UInt32,
    _ outText: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>?,
    _ outTimingsJson: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>?
) -> Int32 {
    guard let ptr = ptr, let samples = samples else { return -1 }
    let bridge = Unmanaged<FluidAudioBridgeInternal>.fromOpaque(ptr).takeUnretainedValue()
    let samplesArray = Array(UnsafeBufferPointer(start: samples, count: Int(sampleCount)))
    do {
        let (text, json) = try bridge.transcribeSamplesTimed(samplesArray)
        if let outText = outText { outText.pointee = strdup(text) }
        if let outTimingsJson = outTimingsJson { outTimingsJson.pointee = strdup(json) }
        return 0
    } catch {
        print("Transcribe samples timed error: \(error)")
        return -1
    }
}
...
public func fluidaudio_streaming_asr_feed(
    ...
    do {
        let partial = try bridge.streamingAsrFeed(samplesArray)
        if let outPartialText = outPartialText {
            outPartialText.pointee = partial.isEmpty ? nil : strdup(partial)
        }
        return 0
    } catch {
