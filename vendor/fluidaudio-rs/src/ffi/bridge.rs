...
extern "C" {
    ...
    fn fluidaudio_transcribe_samples_timed(
        bridge: *mut std::ffi::c_void,
        samples: *const f32,
        sample_count: u32,
        out_text: *mut *mut i8,
        out_timings_json: *mut *mut i8,
    ) -> i32;
...
impl FluidAudioBridge {
    ...
    /// Batch transcribe returning `(text, token_timings_json)`. The JSON is an
    /// array of `{"t":token,"s":startSec,"e":endSec}`. Used by the live-preview
    /// stitcher to map text back to audio positions.
    pub fn transcribe_samples_timed(&self, samples: &[f32]) -> Result<(String, String), String> {
        let mut text_ptr: *mut i8 = std::ptr::null_mut();
        let mut json_ptr: *mut i8 = std::ptr::null_mut();

        let result = unsafe {
            fluidaudio_transcribe_samples_timed(
                self.ptr,
                samples.as_ptr(),
                samples.len() as u32,
                &mut text_ptr,
        ...
        if result != 0 {
            return Err("Timed transcription failed".to_string());
        }

        let text = if text_ptr.is_null() {
            String::new()
        } else {
            let s = unsafe { CStr::from_ptr(text_ptr) }.to_string_lossy().into_owned();
            unsafe { fluidaudio_free_string(text_ptr) };
            s
        };
        let json = if json_ptr.is_null() {
            ...
            unsafe { fluidaudio_free_string(json_ptr) };
            s
        };
    ...
    pub fn streaming_asr_feed(&self, samples: &[f32]) -> Result<Option<String>, String> {
        let mut partial_text_ptr: *mut i8 = std::ptr::null_mut();
        let result = unsafe {
            fluidaudio_streaming_asr_feed(
                self.ptr,
                samples.as_ptr(),
                samples.len() as u32,
        ...
        if result != 0 {
            return Err("Failed to feed samples to streaming ASR".to_string());
        }
