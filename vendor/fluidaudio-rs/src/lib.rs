...
impl FluidAudio {
    ...
    /// Batch transcribe returning `(text, token_timings_json)`, where the JSON is
    /// an array of `{"t":token,"s":startSec,"e":endSec}`. Enables mapping decoded
    /// text back to audio time (used by streaming-preview stitching).
    pub fn transcribe_samples_timed(
        &self,
        samples: &[f32],
    ) -> Result<(String, String), FluidAudioError> {
        self.bridge
            .transcribe_samples_timed(samples)
            .map_err(FluidAudioError::from)
    }
    ...
    /// Returns the running hypothesis after this chunk (confirmed prefix + live
    /// volatile tail joined), or `None` if nothing has been transcribed yet.
    pub fn streaming_asr_feed(
        &self,
        samples: &[f32],
